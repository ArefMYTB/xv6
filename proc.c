...
// Return how many times `read` system call is called
int
getReadCount(void){
  return 0;
}
...
int clone(void *stack)
{
  ...
  //Remember stack grows downwards thus the stacktop will be in the address given by parent
    np->stackTop = (int)((char*)stack + PGSIZE); // DEFINE PGSIZE
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->parent != curproc) // || p->pgdir != curproc->pgdir
        continue;
      if(p->threads != -1) // only wait for child threads not child processes
        continue;
      havekids = 1;
      if(p->state == ZOMBIE){
        // Found one.
